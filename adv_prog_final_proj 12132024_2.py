...
all_countries_inflation['Country'].unique()

gdp_long.head(2)

cpi_years.head(2)
cpi_years['Country'].unique()

gni_long.head(2)
gni_long['Country'].unique()
...
country_dict = {name.replace(' and ', '').replace('&', '').replace(' ', '').strip().lower(): name
                for name in countries_to_keep['Country']}
...
top_gdp = ['Switzerland', 'United States', 'China', 'Japan', 'Germany', 'India', 'United Kingdom', 'France', 'Russia', 'Canada', 'Italy']
...
country_global_df['Country'].unique()
...
#force us household income to only have year and household income again just to be sure
us_household_income = us_household_income[['Year', 'Household Income']]
