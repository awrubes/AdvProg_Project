...
#DROP NaN ROWS
country_merged_df = country_merged_df.dropna(subset=['CPI', 'Inflation', 'GDP per Capita', 'GNI per capita '])
...
print("\nDescriptive Statistics:")
print(country_merged_df[indicators].describe())

# Macroeconomic Trend Analysis

# Calculate the average CPI and Inflation for each country
country_avg = country_merged_df.groupby('Country')[['CPI', 'Inflation']].mean().sort_values(by=['CPI', 'Inflation'], ascending=False)
...
sns.scatterplot(data=country_merged_df, x='Inflation', y='CPI',
...
global_merged_df_means   year interest rate, scpi
...
us_data = country_merged_df[country_merged_df['Country'] == 'United States']

# Create the line plot
plt.figure(figsize=(10, 6))
plt.plot(us_data['Year'], us_data['CPI'], label='CPI')
plt.plot(us_data['Year'], us_data['Inflation'], label='Inflation')
plt.plot(us_data['Year'], us_data['GDP per Capita'], label='GDP per Capita')
plt.plot(us_data['Year'], us_data['GNI per capita '], label='GNI per Capita')
...
plt.xlabel('Year')
plt.ylabel('Value')
plt.title('Macroeconomic Indicators for the United States Over Time')
plt.legend()
plt.grid(True)
