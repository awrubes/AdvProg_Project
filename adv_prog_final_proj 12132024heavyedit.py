...
#tilt the dates for readability
plt.gca().xaxis.set_major_locator(ticker.MaxNLocator(nbins=20))  # the number of labelslabels
plt.xticks(rotation=45)
#dipslay plot
plt.show()
...
def preprocess_countries(df, column_name='Country'):
    ...
    df[column_name] = df[column_name].str.strip().str.replace('&', '').str.lower()
    return df
...
#Convert Country to Camelcase
country_merged_df['Country'] = country_merged_df['Country'].str.title()
country_merged_df
...
country_merged_df['Year'] = country_merged_df['Year'].astype(int)  # Convert to int
global_merged_df_means['Year'] = global_merged_df_means['Year'].astype(int)  # Convert to int

# Year converted from index to column display
print(global_merged_df_means.head())

global_merged_df_means.head()
...
melted_df = country_merged_df.melt(id_vars=['Country', 'Year'],
...
#Transforming the US Household Income data frame

real_income_df_monthly = real_income_df_monthly.reset_index(drop=True)

#need to make this comparable to country dataframe, so either mean or December of each year so it's by year

real_income_df_monthly['Year'] = real_income_df_monthly['Month_year'].apply(lambda x: x.year)

real_income_df_monthly_grouped = real_income_df_monthly.groupby('Year')

us_household_income = real_income_df_monthly_grouped.mean()

us_household_income = us_household_income.drop("Month_year", axis=1)
us_household_income = us_household_income.drop("level_0", axis=1)
us_household_income = us_household_income.drop("index", axis=1)
us_household_income = us_household_income.rename(columns={'MEHOINUSA672N': 'Household Income'})
us_household_income['Household Income']=us_household_income['Household Income'].round(2)


us_household_income.head(5)

us_household_income = real_income_df.rename(columns={"DATE": "Year"}, inplace=True)

real_income_df
#1. Rename the column 'DATE' to 'Year'
real_income_df.rename(columns={"Month_year": "Year"}, inplace=True)

# 2. Extract only the year from the 'Year' column
real_income_df['Year'] = pd.to_datetime(real_income_df['Year']).dt.year

# 3. Rename 'MEHOINUSA672N' to 'Household Income'
real_income_df.rename(columns={"MEHOINUSA672N": "Household Income"}, inplace=True)

# Display the updated DataFrame
print(real_income_df)
us_household_income = real_income_df
us_household_income.columns
# Dhanya

# Merge country_us_df and us_household_income on Year
...
sns.lineplot(
    x=country_us_df['Year'],
    y=country_us_df['Supply Chain Pressure Index'],
